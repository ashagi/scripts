...
INBOX_ITEMS=$(curl -s "http://localhost:41184/folders/$JOPLIN_INBOX_FOLDER_ID/notes?token=$JOPLIN_TOKEN" | jq -r ".items[].title")
...
do
    ...
    -X POST \
    --data "{\"content\": \"Joplin inbox: $TODO\", \"project_id\": $PROJECT_ID, \"due_string\": \"Friday\", \"label_ids\": [$LABEL_ID]}" \
